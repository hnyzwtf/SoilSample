...
public class ThreeLayerCusMdl extends BaseProfileModel implements OnGestureListener{
    ...
    public ThreeLayerCusMdl(Context context, float[] lines, String[] layerName, float sumDepth){
        super(context, lines);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                showDepth = false;
                if (fmlLines[2]- gap < event.getY() && event.getY() < fmlLines[2] + gap) {
                    flag = 2;
...
//			lynum = 0;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < proLegendCodes.length; i++) {
            if(0 == proLegendCodes[i])
                break;
            if (isDraw[i]) {
                DrawLegend.drawCustomChoose(proLegendCodes[i], canvasBitmap, paint, linePaint, width,
                        fmlLines[i], fmlLines[i+1], layerNames[i]);
                if (showDepth) {
                    modifyDepth();
    ...
    public void onLongPress(MotionEvent event) {
        if (event.getY()> fmlLines[2]+20 && event.getY()<fmlLines[3]-20) {
            lynum = 3;
    ...
    public int[] getProLegendCode() {
        return proLegendCodes;
    }

    public void setProLegendCode(int[] proLegendCode) {
        this.proLegendCodes = proLegendCode;
    }
