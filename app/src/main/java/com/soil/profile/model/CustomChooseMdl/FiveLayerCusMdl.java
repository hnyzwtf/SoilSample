...
public class FiveLayerCusMdl extends BaseProfileModel implements OnGestureListener{
    ...
    int gap = 10;
    int legend;
    int lynum;//用于标记长按时选中的土层

    public FiveLayerCusMdl(Context context, float[] lines, String[] layerName, float sumDepth){
        super(context, lines);
    ...
    public void setPro_name(int[] proLegendCode) {
        this.proLegendCodes = new int[proLegendCode.length];
        for (int i = 0; i < proLegendCode.length; i++) {
            this.proLegendCodes[i] = proLegendCode[i];
        }
    }
