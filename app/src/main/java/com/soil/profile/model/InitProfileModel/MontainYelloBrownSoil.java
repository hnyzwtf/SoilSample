...
public class MontainYelloBrownSoil extends BaseProfileModel implements OnGestureListener{

    private static float one = (float)5/80;
    private static float two = (float)20/80;
    private static float three = (float)60/80;

    private static float[] lines = {0, one, two, three, 1};
    int[] proLegendCode = {2, 1, 9, 11};
    String[] layerNameAndDepth = {"O\t ", "A\t ", "B\t ", "C\t "};
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if (fmlLines[3]-gap < event.getY() && event.getY() < fmlLines[3] + gap) {
                    flag = 3;
...
//			lynum = 0;
    ...
    public List<Map<String, Object>> getLayerInfo(float length){
        ...
        o.put("name", "枯枝落叶层(O)");
        o.put("length", length*5/80);
        infoList.add(o);
        Map<String, Object> a = new HashMap<String, Object>();
        a.put("name", "腐殖质层(A)");
        a.put("length", length*15/80);
        infoList.add(a);
        Map<String, Object> b = new HashMap<String, Object>();
        b.put("name", "沉淀层(B)");
        b.put("length", length*40/80);
        infoList.add(b);
        Map<String, Object> c = new HashMap<String, Object>();
        c.put("name", "母质质层(C)");
        c.put("length", length*20/80);
        infoList.add(c);
        return infoList;
    }
    @Override
    ...
    public void onLongPress(MotionEvent event) {
        if (event.getY()> fmlLines[3]+20 && event.getY()<fmlLines[4]-20) {
            lynum = 4;
